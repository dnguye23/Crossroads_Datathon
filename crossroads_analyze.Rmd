...
p <- ggplot(df, aes(x=BusinessDate, y=TotalCashUsed)) +
       geom_line()
...
df1 <- df %>%
  group_by(BusinessDate, BranchID) %>%
  summarize(meanTotalCash = mean(TotalCashUsed))

p <- ggplot(df1, aes(x=month(BusinessDate), y=meanTotalCash)) +
       geom_line()
